# 공모주 청약 캘린더

공모주 판단에 필요한 숫자를 한 페이지에 모아 보여주는 정적 사이트입니다.
공시 원문을 매일 자동 수집해 `data/ipos.json` 으로 만들고, GitHub Pages로 배포합니다.

- 청약 일정 (수요예측 · 청약 · 납입 · 환불 · 상장)
- 수요예측 결과 (기관경쟁률, 가격대별 참여 분포)
- 의무보유확약 비율과 기간별(6개월/3개월/1개월/15일) 분포
- 공모 청약 경쟁률 (단순 · 비례)
- 회사 개요 (업종 · 대표자 · 자본금 · 매출 · 손익 · 소재지)
- 공모자금 사용목적 *(DART API 키 등록 시)*

## 데이터 출처

| 항목 | 출처 | 성격 |
|---|---|---|
| 시장구분, 신고서 제출일, 일정, 확정공모가, 공모금액, 주관사 | [KIND](https://kind.krx.co.kr/listinvstg/pubofrprogcom.do?method=searchPubofrProgComMain) (한국거래소) | 공식 |
| 공모자금 사용목적, 정식 회사개요, 증권신고서 링크 | [OpenDART](https://opendart.fss.or.kr/) (금융감독원) | 공식 API |
| 희망공모가 밴드, 기관경쟁률, 의무보유확약, 가격 분포, 청약경쟁률, 배정 물량 | [38커뮤니케이션](http://www.38.co.kr/html/fund/) | 보조 |

일정·공모가·공모금액처럼 공식 소스가 가진 값은 **KIND를 우선**하고, KIND가 아직
반영하지 못한 임박 공모나 KIND에 없는 상세 지표만 보조 소스로 채웁니다.
각 종목 상세의 "원문 확인"에서 출처 페이지로 바로 이동할 수 있습니다.

**상태(청약중/청약예정 등)와 D-day는 화면에서 다시 계산합니다.** `data/ipos.json` 에도
수집 시점 기준 값이 들어 있지만, 사이트는 페이지를 여는 순간의 **서울 기준 날짜**로
재계산하므로 자동 갱신 사이에 날짜가 바뀌어도 어긋나지 않습니다. 갱신이 이틀 이상
밀리면 화면 하단에 경고가 뜹니다.

## 로컬에서 실행

```bash
pip install -r requirements.txt
python3 -m collector.run --pages 3
python3 -m http.server 8765
```

브라우저에서 <http://localhost:8765> 를 엽니다.

| 옵션 | 뜻 |
|---|---|
| `--pages N` | 38커뮤니케이션 목록을 N페이지까지 훑음 (기본 3, 약 90종목) |
| `--min-year Y` | KIND 최소 신고 연도 (기본 2025) |
| `--limit N` | 상세 수집 상한 — 빠른 테스트용 |
| `--no-dart` | DART 보강 생략 |

전체 수집은 약 2분 걸립니다. 서버에 부담을 주지 않도록 호스트별로 요청 간격을 둡니다.

## GitHub Pages 배포

1. 이 폴더를 GitHub 저장소로 push 합니다.
2. **Settings → Pages → Source** 를 `Deploy from a branch`, 브랜치 `main` / 폴더 `/ (root)` 로 지정합니다.
3. 몇 분 뒤 `https://<계정>.github.io/<저장소>/` 에서 열립니다.

## 자동 갱신

`.github/workflows/update.yml` 이 매일 **KST 07:30** 한 번 돌면서 `data/ipos.json` 을
갱신하고, 변경이 있을 때만 커밋합니다. 저장소 Actions 탭에서 수동 실행도 됩니다.

전날 청약 마감과 수요예측 결과가 모두 확정된 뒤에 돌기 때문에, 하루치 변화를 한 번에
담습니다. 다만 **당일 오후에 마감되는 청약의 경쟁률은 다음 날 아침에 반영**됩니다.
더 빨리 보고 싶으면 Actions 탭에서 `Run workflow` 로 즉시 돌릴 수 있습니다.

날짜 기준은 항상 **서울(Asia/Seoul)** 입니다. GitHub 러너는 UTC로 도는데, KST 07:30 은
UTC로 전날 22:30 이라 그냥 두면 `generated_at` 이 하루씩 밀립니다.

## 공모자금 사용목적 켜기

이 항목만 DART 증권신고서 원문에 있어 API 키가 필요합니다.

1. <https://opendart.fss.or.kr/> 에서 무료 인증키를 발급받습니다.
2. 저장소 **Settings → Secrets and variables → Actions → New repository secret**
   에 이름 `DART_API_KEY` 로 등록합니다.
3. 로컬에서는 `export DART_API_KEY=발급받은키` 후 실행합니다.

키가 없으면 이 항목만 비어 있고 나머지는 정상 동작합니다.

## 카카오톡 알림 (나에게 보내기)

매일 아침 자동 갱신이 끝나면 오늘의 공모주 요약을 **내 카카오톡('나와의 채팅')으로**
보냅니다. 청약중이거나 3일 안에 청약이 시작되는 종목이 있을 때만 보내고, 조용한 날은
보내지 않습니다.

카카오는 **자기 자신에게 보내는 것만** 앱 검수 없이 허용합니다. 다른 사람에게 보내려면
별도 검수가 필요합니다.

### 최초 1회 설정

[카카오 개발자](https://developers.kakao.com) 에서 먼저:

1. **내 애플리케이션 > 애플리케이션 추가하기**
2. **앱 설정 > 플랫폼 > Web** — 사이트 도메인에 `https://localhost`
3. **제품 설정 > 카카오 로그인** — 활성화 ON
4. **제품 설정 > 카카오 로그인 > Redirect URI** — `https://localhost` 등록
5. **동의항목** — `카카오톡 메시지 전송(talk_message)` 을 **이용 중 동의** 로
6. **앱 설정 > 앱 키 > REST API 키** 복사

그다음 터미널에서:

```bash
python3 -m notify.setup_kakao
```

안내대로 진행하면 리프레시 토큰이 나옵니다. 저장소 Secrets 에 두 개를 등록하세요.

| Secret 이름 | 값 |
|---|---|
| `KAKAO_REST_API_KEY` | REST API 키 |
| `KAKAO_REFRESH_TOKEN` | 스크립트가 출력한 리프레시 토큰 |

### 메시지 미리보기

실제로 보내지 않고 내용만 확인하려면:

```bash
python3 -m notify.message
```

### 알림이 끊겼다면

리프레시 토큰은 60일마다 갱신이 필요합니다. 카카오가 새 토큰을 발급하면 Actions 로그에
경고와 함께 새 값이 찍히니, `KAKAO_REFRESH_TOKEN` Secret 을 그 값으로 바꾸세요.
놓쳐서 만료됐다면 `python3 -m notify.setup_kakao` 를 다시 실행하면 됩니다.

## 공모주 소개글 트리거 (D-4)

청약 시작 4일 전인 종목을 뽑아 블로그 소개글 작성 대상으로 알려준다.

```bash
python3 -m notify.dday            # 오늘 기준 D-4
python3 -m notify.dday --days 2   # D-2
python3 -m notify.dday --spac     # 스팩 포함
```

매일 아침 카카오톡 알림에도 `✍️ D-4 소개글` 줄로 함께 나간다.

**글쓰기 자체는 자동화하지 않는다.** GitHub Actions 러너에는 글을 쓸 모델이 없기
때문이다. 이 스크립트는 "오늘 어느 종목 글을 써야 하는가"까지만 확정하고, 실제
집필은 로컬에서 Claude 에게 이 출력을 넘겨 진행한다.

D-4 시점에는 확정공모가·기관경쟁률·의무보유확약이 아직 공시 전인 경우가 많다.
없는 숫자를 추정해 채우지 말고, 언제 무엇이 공시되는지를 확인 지점으로 제시해
마무리한다. 스팩은 공모가 2,000원 고정에 합병 대상이 미정이라 종목별 개별 글로
쓰면 내용이 겹치므로 기본 제외한다.

## 주의

공시 자료를 자동 수집한 것으로, 정정신고나 원본 입력 오류로 실제와 다를 수 있습니다.
투자 판단과 책임은 투자자 본인에게 있으며, 이 페이지는 투자 권유가 아닙니다.

## 구조

```
index.html            화면
assets/               스타일 · 렌더링 스크립트
collector/
  kind.py             KIND 공모기업 현황
  ipo38.py            38커뮤니케이션 상세 지표
  dart.py             OpenDART 사용목적 · 회사개요
  merge.py            병합 규칙과 파생지표(상태, D-day, 밴드 위치)
  run.py              파이프라인 진입점
notify/
  message.py          오늘 알아야 할 것만 200자로 압축
  kakao.py            카카오톡 '나에게 보내기' 전송
  setup_kakao.py      최초 1회 리프레시 토큰 발급 (직접 실행)
data/ipos.json        수집 결과 (사이트가 읽는 파일)
```

브라우저 캐시 때문에 수정이 반영되지 않으면 `index.html` 의 `assets/...?v=1` 숫자를 올려주세요.
